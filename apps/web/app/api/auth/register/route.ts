...
export async function POST(request: NextRequest) {
  ...
  const user = await prisma.user.create({ data: { email: body.data.email.toLowerCase(), passwordHash: await hash(body.data.password, 12), profile: { create: { fullName: body.data.fullName, location: "", skills: [], titles: [], yearsExperience: 0 } }, preferences: { create: { targetRoles: [], locations: [], employmentTypes: [] } } } });
